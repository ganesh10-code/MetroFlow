...
export default function MaintenanceDashboard() {
  ...
  const submitToday = async () => {
    try {
      ...
      await api.post("/maintenance/submit", Object.values(draftRows));
  ...
  return (
    <Layout>
      ...
      {/* ADD ENTRY */}
      <Card title="Train Maintenance Input">
          ...
          <div className="grid md:grid-cols-4 gap-5">
            ...
            <div>
              ...
              <input
                ...
                value={openJobs}
                disabled={locked}
                onChange={(e) => setOpenJobs(e.target.value)}
                className="w-full rounded-xl border border-slate-700 bg-slate-800 px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-cyan-500"
              />
            </div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full rounded-xl border border-slate-700 bg-slate-800 px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-cyan-500"
              >
                <option className="text-white">LOW</option>

                <option className="text-white">MEDIUM</option>

                <option className="text-white">HIGH</option>
              </select>
            </div>
              ...
              <tbody>
                {Object.values(draftRows).map((r) => (
                  <tr key={r.train_id} className="border-b border-slate-800">
                    ...
                    <td className="py-2 pr-4">{r.open_jobs}</td>

                    <td className="py-2 pr-4">{r.urgency_level}</td>
      ...
      <Card title="Today's Submitted Data">
        {todayRows.length === 0 ? (
          ...
          <Table
            headers={["Train", "Open Jobs", "Urgency", "Updated At"]}
